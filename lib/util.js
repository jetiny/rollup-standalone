...
export default ({re, json, resolve, commonjs, rollup, babel, vue, include}) => {
  ...
  function createPlugins ({patterns, defines, replaces, babelOptions, uglifyOptions, vueOptions, resolveOptions, commonjsOptions, includeOptions}, ongenerate) {
    let restore = patterns ? patterns.filter((it) => 'restore' in it) : []
    restore = restore.length ? restoreCode(restore) : false
    let ret = []
    if (includeOptions) {
      ret.push(include(includeOptions))
    }
    ret = ret.concat([
      re({
        defines,
